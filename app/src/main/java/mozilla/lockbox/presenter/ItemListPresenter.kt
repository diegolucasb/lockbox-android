...
class ItemListPresenter(
    ...
    fun onMenuItem(@IdRes item: Int) {
        when (item) {
            R.id.action_itemList_to_locked, R.id.fragment_locked -> dispatcher.dispatch(RouteAction.LockScreen)
            R.id.action_itemList_to_setting, R.id.fragment_setting -> dispatcher.dispatch(RouteAction.SettingList)
            else -> log.error("Cannot route from item list menu")
        }
    }
}